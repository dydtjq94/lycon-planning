...
export function calculateInflatedAmount(
  ...
  return Math.round(
    currentAmount * Math.pow(1 + inflationRate / 100, yearsToRetirement),
  );
}
...
export function calculatePostRetirementExpense(
  ...
  const inflatedAmount = calculateInflatedAmount(
  ...
  switch (category) {
    case "income":
      return {
        ...baseItem,
        title:
          ADDABLE_ITEM_TYPES.income.find((t) => t.type === type)?.label ||
          "소득",
        end_year: retirementYear,
        is_fixed_to_retirement_year: true,
        data: {
    ...
    case "expense":
      return {
        ...baseItem,
        title:
          ADDABLE_ITEM_TYPES.expense.find((t) => t.type === type)?.label ||
          "지출",
        owner: "common",
        data: {
          amount: 0,
    ...
    case "savings":
      return {
        ...baseItem,
        title:
          ADDABLE_ITEM_TYPES.savings.find((t) => t.type === type)?.label ||
          "저축",
        owner: "common",
        data: {
          currentBalance: 0,
    ...
    case "pension":
      return {
        ...baseItem,
        title:
          ADDABLE_ITEM_TYPES.pension.find((t) => t.type === type)?.label ||
          "연금",
        start_year:
          type === "national"
            ? birthYear + DEFAULT_RATES.nationalPensionStartAge
            : undefined,
        data: {
          currentBalance: 0,
          returnRate: DEFAULT_RATES.pensionReturn,
          paymentStartAge:
            type === "national"
              ? DEFAULT_RATES.nationalPensionStartAge
              : undefined,
        } as PensionData,
      } as FinancialItemInput;
    ...
    case "debt":
      return {
        ...baseItem,
        title:
          ADDABLE_ITEM_TYPES.debt.find((t) => t.type === type)?.label || "대출",
        owner: "common",
        data: {
          principal: 0,
          currentBalance: 0,
    ...
    case "real_estate":
      return {
        ...baseItem,
        title:
          ADDABLE_ITEM_TYPES.real_estate.find((t) => t.type === type)?.label ||
          "부동산",
        owner: "common",
        data: {
          currentValue: 0,
