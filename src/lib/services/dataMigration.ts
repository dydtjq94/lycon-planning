...
const DEFAULT_RATES = {
  ...
  expenseGrowth: 2.5, // 지출 증가율 (물가 상승률)
...
function getDefaultMaturity(): { year: number; month: number } {
  ...
  const endYear =
    currentYear + Math.floor((currentMonth - 1 + DEFAULT_LOAN_MONTHS) / 12);
...
export function migrateOnboardingToFinancialItems(
  ...
  const spouseRetirementAge =
    data.spouse?.retirement_age || data.target_retirement_age || 60;
  ...
  if (data.laborIncome && data.laborIncome > 0) {
    items.push({
      ...
      data: {
        amount:
          data.laborIncomeFrequency === "yearly"
  ...
  if (data.spouseLaborIncome && data.spouseLaborIncome > 0) {
    items.push({
      ...
      data: {
        amount:
          data.spouseLaborIncomeFrequency === "yearly"
  ...
  if (data.businessIncome && data.businessIncome > 0) {
    items.push({
      ...
      data: {
        amount:
          data.businessIncomeFrequency === "yearly"
  ...
  if (data.spouseBusinessIncome && data.spouseBusinessIncome > 0) {
    items.push({
      ...
      data: {
        amount:
          data.spouseBusinessIncomeFrequency === "yearly"
  ...
  if (data.livingExpenses && data.livingExpenses > 0) {
    const monthlyLiving =
      data.livingExpensesFrequency === "yearly"
    ...
    // 은퇴 후 생활비 (은퇴 전의 70% 수준, 물가 상승률 반영)
    const yearsToRetirement = retirementYear - currentYear;
    const inflatedLiving = Math.round(
      monthlyLiving *
        Math.pow(1 + DEFAULT_RATES.expenseGrowth / 100, yearsToRetirement),
    );
  ...
  if (selfMedicalExpense > 0) {
    items.push({
      ...
      data: {
        ...
        frequency: "monthly",
        growthRate: 5.0, // 물가 상승률 + 나이 증가에 따른 의료비 상승
      } as ExpenseData,
  ...
  if (data.housingType && data.housingType !== "해당없음") {
    ...
    if (data.housingHasLoan && data.housingLoan) {
      ...
      realEstateData.loanRepaymentType =
        data.housingLoanType || "원리금균등상환";
    ...
    const housingStartYear =
      data.housingType === "자가" && data.housingPurchaseYear
        ? data.housingPurchaseYear
        : currentYear;
    const housingStartMonth =
      data.housingType === "자가" && data.housingPurchaseMonth
  ...
  if (data.realEstateProperties && data.realEstateProperties.length > 0) {
    data.realEstateProperties.forEach((property) => {
      ...
      if (property.hasLoan && property.loanAmount) {
        ...
        propData.loanRepaymentType =
          property.loanRepaymentType || "원리금균등상환";
    ...
    data.investmentAccounts.forEach((account) => {
      if (account.balance && account.balance > 0) {
        ...
        const typeMap: Record<
          string,
          "stock" | "fund" | "crypto" | "savings_account"
        > = {
        ...
        items.push({
          ...
          data: {
            currentBalance: account.balance,
            interestRate:
              account.expectedReturn || DEFAULT_RATES.investmentReturn,
          } as SavingsData,
        });
      }
    });
  ...
  const hasHousingDebtInDebts = data.debts?.some(
    (d) => d.sourceType === "housing",
  );
  ...
  if (data.debts && data.debts.length > 0) {
    data.debts.forEach((debt, index) => {
      if (debt.amount && debt.amount > 0) {
        ...
        let debtType: "mortgage" | "credit_loan" | "car_loan" | "other" =
          "credit_loan";
  ...
  if (
    data.isMarried &&
    data.spouseNationalPension &&
    data.spouseNationalPension > 0
  ) {
  ...
  if (hasRetirementPension) {
    const retirementStartAge =
      data.retirementPensionStartAge || data.target_retirement_age || 60;
  ...
  const hasSpouseRetirementPension =
    (data.spouseRetirementPensionType === "DB" &&
      data.spouseYearsOfService &&
      data.spouseYearsOfService > 0) ||
    (data.spouseRetirementPensionBalance &&
      data.spouseRetirementPensionBalance > 0) ||
    data.spouseRetirementPensionType;

  if (hasSpouseRetirementPension && data.isMarried) {
    const spouseRetirementStartAge =
      data.spouseRetirementPensionStartAge || spouseRetirementAge;
    const spouseRetirementPensionStartYear =
      spouseBirthYear + spouseRetirementStartAge; // 배우자 생년 기준
    const spouseRetirementReceivingYears =
      data.spouseRetirementPensionReceivingYears || 10;
    ...
    items.push({
      ...
      data: {
        ...
        receiveType:
          data.spouseRetirementPensionReceiveType === "annuity"
            ? "annuity"
            : "lump_sum",
  ...
  // IRP (본인) - 연금 자산 항목만 생성
  const hasIrp =
    (data.irpBalance && data.irpBalance > 0) ||
    (data.irpMonthlyContribution && data.irpMonthlyContribution > 0);
  if (hasIrp) {
  ...
  const hasSpouseIrp =
    data.isMarried &&
    ((data.spouseIrpBalance && data.spouseIrpBalance > 0) ||
      (data.spouseIrpMonthlyContribution &&
        data.spouseIrpMonthlyContribution > 0));
  if (hasSpouseIrp) {
  ...
  const hasPensionSavings =
    (data.pensionSavingsBalance && data.pensionSavingsBalance > 0) ||
    (data.pensionSavingsMonthlyContribution &&
      data.pensionSavingsMonthlyContribution > 0);
  if (hasPensionSavings) {
    ...
    const pensionSavingsReceivingYears =
      data.pensionSavingsReceivingYears || 10;
  ...
  const hasSpousePensionSavings =
    data.isMarried &&
    ((data.spousePensionSavingsBalance &&
      data.spousePensionSavingsBalance > 0) ||
      (data.spousePensionSavingsMonthlyContribution &&
        data.spousePensionSavingsMonthlyContribution > 0));
  if (hasSpousePensionSavings) {
    const spousePensionSavingsStartAge =
      data.spousePensionSavingsStartAge || 56;
    const spousePensionSavingsStartYear =
      spouseBirthYear + spousePensionSavingsStartAge; // 배우자 생년 기준
    const spousePensionSavingsReceivingYears =
      data.spousePensionSavingsReceivingYears || 10;
