...
function mapAssetToSavings(
  ...
  return {
    ...
    title: item.title,
    broker_name: metadata.brokerName || null,
...
function mapPensionToPersonalPension(
  ...
  return {
    ...
    start_age: metadata.startAge || 55,
    receiving_years: metadata.receivingYears || 20,
    return_rate: metadata.expectedReturn || 5.0,
    isa_maturity_year: item.item_type === 'isa' ? (metadata.maturityYear || null) : null,
    isa_maturity_month: item.item_type === 'isa' ? (metadata.maturityMonth || null) : null,
    isa_maturity_strategy: item.item_type === 'isa' ? (metadata.maturityStrategy || 'pension_savings') : null,
    is_active: true,
  }
}
...
function mapDebtToSimulation(
  ...
  const maturityYear = metadata.maturityYear || currentYear + 10
  const maturityMonth = metadata.maturityMonth || 12
  ...
  return {
    ...
    current_balance: item.amount,
    interest_rate: metadata.interestRate || 5.0,
    rate_type: metadata.rateType || 'fixed',
    repayment_type: metadata.repaymentType || '원리금균등상환',
    start_year: metadata.startYear || currentYear,
    start_month: metadata.startMonth || currentMonth,
...
function mapRealEstateToSimulation(
  ...
  return {
    ...
    purchase_price: metadata.purchasePrice || item.amount,
    purchase_year: metadata.purchaseYear || currentYear,
    purchase_month: metadata.purchaseMonth || currentMonth,
    growth_rate: metadata.growthRate || 3.0,
    housing_type: metadata.housingType || '자가',
    deposit: metadata.deposit || null,
    monthly_rent: metadata.monthlyRent || null,
    has_loan: metadata.hasLoan || false,
    loan_amount: metadata.loanAmount || null,
    loan_rate: metadata.loanRate || null,
    is_active: true,
    sort_order: item.sort_order || 0,
  }
}
