...
import { DiagnosisData, FixedExpenseItem, CalculationParams, DEFAULT_CALC_PARAMS, calculateAllDiagnosisMetrics, calculateAdditionalCosts } from "@/lib/services/diagnosisDataService";
...
export function ReportTabs({ data, opinion }: ReportTabsProps) {
  ...
  const [activeSheet, setActiveSheet] = useState<"cost" | "assumption" | null>(null);
  ...
  return (
    <div className={styles.tabsContainer}>
        ...
        {activeTab === "summary" && (
          <>
            ...
            <div className={styles.stepCard}>
              ...
              <div className={styles.keyMetricsGrid}>
                <div className={styles.keyMetricItem}>
                  ...
                  <span className={styles.keyMetricValue}>{formatBillion(m.netWorth)}억</span>
                ...
                <div className={styles.keyMetricItem}>
                  ...
                  <span className={styles.keyMetricValue}>{Math.round(m.savingsRate)}%</span>
                  <span className={`${styles.keyMetricBadge} ${m.currentMonthlyGap >= 0 ? styles.sufficient : styles.critical}`}>
                    월 {m.currentMonthlyGap >= 0 ? "+" : ""}{m.currentMonthlyGap}만
                  </span>
                </div>
                <div className={styles.keyMetricItem}>
                  <span className={styles.keyMetricLabel}>은퇴까지</span>
                  <span className={styles.keyMetricValue}>{m.yearsToRetirement}년</span>
                  <span className={styles.keyMetricBadge}>{m.effectiveRetirementAge}세 목표</span>
                </div>
                <div className={styles.keyMetricItem}>
                  <span className={styles.keyMetricLabel}>고정비:변동비</span>
                  <span className={styles.keyMetricValue}>
                    {(m.fixedExpense + m.variableExpense) > 0 ? Math.round((m.fixedExpense / (m.fixedExpense + m.variableExpense)) * 100) : 0}:{(m.fixedExpense + m.variableExpense) > 0 ? Math.round((m.variableExpense / (m.fixedExpense + m.variableExpense)) * 100) : 0}
                  </span>
                  <span className={styles.keyMetricBadge}>{m.fixedExpense}만/{m.variableExpense}만</span>
                </div>
              </div>
            </div>
            ...
            <div className={styles.stepCard}>
              ...
              <div className={styles.pensionSummaryGrid}>
                <div className={styles.pensionSummaryItem}>
                  ...
                  <span className={styles.pensionSummaryValue}>{(data.nationalPensionPersonal + data.nationalPensionSpouse).toLocaleString()}만/월</span>
                </div>
                <div className={styles.pensionSummaryItem}>
                  ...
                  <span className={styles.pensionSummaryValue}>{(data.retirementPensionBalanceSelf + data.retirementPensionBalanceSpouse).toLocaleString()}만</span>
                </div>
                <div className={styles.pensionSummaryItem}>
                  ...
                  <span className={styles.pensionSummaryValue}>
                    {(data.personalPensionStatus.irp.balance + data.personalPensionStatus.pensionSavings.balance + data.personalPensionStatus.isa.balance).toLocaleString()}만
                  </span>
                </div>
              </div>
            </div>
            ...
            {(() => {
              ...
              const educationCost = costOptions.education === "none" ? 0
                : costOptions.education === "normal" ? additionalCosts.childEducation.grandTotalNormal
                : additionalCosts.childEducation.grandTotalPremium;
              const leisureCost = costOptions.leisure === "none" ? 0
              ...
              const consumerGoodsCost = costOptions.consumerGoods === "none" ? 0
                : additionalCosts.consumerGoods[costOptions.consumerGoods as number]?.totalUntilLifeExpectancy || 0;
              const medicalCost = costOptions.medical ? additionalCosts.medical.grandTotal : 0;
              const housingCost = costOptions.housing
                ...
                : 0;
              const additionalTotal = educationCost + medicalCost + leisureCost + consumerGoodsCost + housingCost;
              ...
              return (
                <div className={styles.stepCard}>
                  ...
                  <div className={styles.stepMainResult}>
                    <div className={styles.stepMainValue}>{formatBillion(totalNeed)}<span className={styles.stepMainUnit}>억</span></div>
                  ...
                  <div className={styles.fundingBreakdown}>
                    <div className={styles.fundingBreakdownItem}>
                      ...
                      <span className={styles.fundingBreakdownValue}>{formatBillion(m.totalDemand)}억</span>
                    </div>
                    <span className={styles.fundingBreakdownPlus}>+</span>
                    <div className={styles.fundingBreakdownItem}>
                      <span className={styles.fundingBreakdownLabel}>추가</span>
                      <span className={styles.fundingBreakdownValue}>{formatBillion(additionalTotal / 10000)}억</span>
                    </div>
                  </div>
                </div>
              );
      ...
      {/* 바텀시트 오버레이 */}
      {activeSheet && (
        <div className={styles.bottomSheetOverlay} onClick={() => setActiveSheet(null)} />
      )}

      {/* 비용 변경 바텀시트 */}
      <div className={`${styles.bottomSheet} ${activeSheet === "cost" ? styles.open : ""}`}>
        <div className={styles.bottomSheetHeader}>
          <span className={styles.bottomSheetTitle}>비용 변경</span>
          <button className={styles.bottomSheetClose} onClick={() => setActiveSheet(null)}>
      ...
      {/* 가정 변경 바텀시트 */}
      <div className={`${styles.bottomSheet} ${activeSheet === "assumption" ? styles.open : ""}`}>
        <div className={styles.bottomSheetHeader}>
