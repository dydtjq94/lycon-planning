...
// 은퇴까지 남은 년수
export function yearsToRetirement(
  birthDate: string,
...
export function calculateRetirementProgress(data: OnboardingData): number {
  if (!data.target_retirement_fund || data.target_retirement_fund === 0)
    return 0;
  const netWorth = calculateNetWorth(data);
  return Math.min(
    100,
    Math.round((netWorth / data.target_retirement_fund) * 100),
  );
}
...
export function calculateRequiredMonthlySaving(data: OnboardingData): number {
  ...
  const yearsLeft = yearsToRetirement(
    data.birth_date,
    data.target_retirement_age,
  );
...
export function calculatePensionReplacement(data: OnboardingData): number {
  ...
  const personalPension =
    (data.irpBalance || 0) + (data.pensionSavingsBalance || 0);
 ...
 * @param years 기간 (년)
 * @param inflationRate 물가 상승률 (%, 기본 2%)
 */
...
export function compareScenarios(data: OnboardingData): ScenarioResult[] {
  ...
  const monthlySavings =
    calculateMonthlyIncome(data) - calculateMonthlyExpense(data);
  ...
  return scenarios.map((scenario) => {
    ...
    const achievementRate =
      targetFund > 0 ? Math.round((projectedFund / targetFund) * 100) : 0;
...
export function compareNationalPensionTiming(
  ...
  return timings.map((timing) => {
    const adjustedMonthly = Math.round(
      monthlyPension * (1 + timing.adjustmentRate / 100),
    );
...
export function analyzeIncomeStability(data: OnboardingData): {
  ...
  const currentEmergencyMonths =
    monthlyExpense > 0 ? Math.round(cash / monthlyExpense) : 0;
...
export function analyzeRetirementGap(data: OnboardingData): {
  ...
  const pensionStartAge =
    data.nationalPensionStartAge || DEFAULT_PENSION_START_AGE;
