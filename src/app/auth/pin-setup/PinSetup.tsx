...
import styles from "./pin-setup.module.css";
  ...
  const handleSubmit = async (finalPin: string) => {
    ...
    try {
      ...
      if (updateError) {
        ...
        if (upsertError) {
          console.error("PIN 저장 오류:", upsertError);
          setError("PIN 저장에 실패했습니다. 다시 시도해주세요.");
          setLoading(false);
          return;
        }
      }
      ...
      {ripples.map((ripple) => (
        <div
          ...
          className={`${styles.globalRipple} ${
            ripple.small ? styles.smallRipple : ""
          }`}
          style={{
            left: ripple.x,
            top: ripple.y,
        ...
        <div className={styles.keypad}>
          <div className={styles.keypadGrid}>
            {keypadNumbers.slice(0, 9).map((num) => (
              <button
                ...
                className={`${styles.keypadButton} ${
                  glowButtons.includes(num) ? styles.glow : ""
                }`}
            ...
            <button
              ...
              className={`${styles.keypadButton} ${
                glowButtons.includes(keypadNumbers[9]) ? styles.glow : ""
              }`}
